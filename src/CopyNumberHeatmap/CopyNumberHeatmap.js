...
const getSelectedAnalysisCellStatsFromIndices = (cellStats, indices) => {
  return cellStats.filter(
    (stat) =>
      stat.heatmap_order >= indices[0] &&
      stat.heatmap_order <= indices[indices.length - 1]
  );
};
...
const CopyNumberHeatmap = ({
  ...
  const [indices, setIndices] = useState([
    ...getIndicesFromAllHeatmapOrder(heatmapConfig, allHeatmapOrder),
  ]);
  ...
  const selectedAnalysisStats = {
    maxState: analysisStats.maxState,
    cellStats: getSelectedAnalysisCellStatsFromIndices(
      analysisStats.cellStats,
      indices
    ),
  };
  return (
