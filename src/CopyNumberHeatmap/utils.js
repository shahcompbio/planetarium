...
export const getChromPixelMapping =(heatmapConfig, chromosomes) => {
  const bpRatio = getBPRatio(heatmapConfig,chromosomes);
  const miniBpRatio = getMinimapBPRatio(heatmapConfig, chromosomes);
...
export const getMinimapBPRatio = (heatmapConfig, chromosomes) => {
 ...
 */
export const getBPRatio = (heatmapConfig,chromosomes) => {
  const totalBP = getTotalBP(chromosomes);
...
export const drawChromosomeAxis= (context, chromosomes, chromMap, categoryWidth, y) =>{
      ...
      chromosomes.forEach((chromosome,index) =>{
        ...
        //text position
        const text_width = currData["x"] + currData["width"] / 2 
        context.fillStyle = fill;
        context.fillRect(x,y + (.013 * heatmapConfig.height) ,width,height)
        context.stroke()
        context.closePath();
        context.fill()
        context.fillStyle = "#000000"
        context.textAlign = "center";
        context.fillText(chromosomeID, text_width , y + 7 + (heatmapConfig.chromosome.height * .8));
